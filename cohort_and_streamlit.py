...
st.write('Os novos clientes que você está adquirindo são mais (ou menos) valiosos do que os usuários anteriores?')
st.write('As alterações que você fez em seu site afetaram positiva ou negativamente a jornada do seu cliente?')
st.write('Existem diferenças sazonais entre os usuários que você adquire?')
st.write('Qual é a sua taxa de retenção?')
st.write('Quando os usuários começam em churn?')
st.write('Será que os clientes que você adquiriu no mês passado agem de forma diferente daqueles que se inscreveram no mês anterior?')
st.write('Os usuários que responderam a um desconto ou promoção se comportam de maneira semelhante ou não daqueles que compraram pelo preço integral?',"")
...
st.write('Identificar problemas mais cedo: você pode ver tendências nos cohorts mais recentes sem correr o risco dos resultados serem mascarados pelo restante da base de dados;',"")
st.write('Criar previsões mais assertivas: conhecendo os comportamentos sazonais, você pode incorporá-los às suas expectativas de performance futura;',"")
st.write('Saber como os comportamentos dos usuários afetam seu negócio: veja como as ações realizadas ou não por seus cohorts se traduzem em mudanças nas métricas da sua empresa, como aquisição e retenção;')
st.write('Entender o churn do seu cliente: avalie hipóteses para entender quais ações incentivam uma maior rotatividade;')
st.write('Calcular o valor da vida útil do cliente: veja quanto os seus clientes valem para a empresa ao longo do tempo. Você pode, inclusive, agrupar ainda mais esses cohorts para avaliar quais canais de aquisição levam ao melhor valor de vida útil do cliente (LTV);')
st.write('Otimizar seu funil de conversão: descubra como a experiência do usuário em todo o funil de marketing digital se traduz em valor para seus clientes;')
st.write('Criar um relacionamento mais eficaz com o cliente: à medida que você identifica padrões, fica mais fácil se relacionar com os seus usuários.')
st.write('A análise responde tantas perguntas que permite que uma empresa identifique padrões claros em diferentes grupos de clientes')
